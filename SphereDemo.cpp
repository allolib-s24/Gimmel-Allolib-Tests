...
#include "Gimmel/test/JaffMalloc.hpp"
Jaffx::MyMalloc m;
...
private:
    ...
    std::unique_ptr<giml::Reverb<float>> reverb;
...
public:
	...
	detune(sampleRate), compressor(sampleRate), saturation(sampleRate), filter(sampleRate), tremolo(sampleRate), chorus(sampleRate) {
		filter.setType(giml::Biquad<float>::BiquadUseCase::LPF_1st);
        reverb = std::make_unique<giml::Reverb<float>>(this->sampleRate);
	}
	...
	void onCreate() override {
		...
		////Set up the effects line here
		//line.pushBack(this->saturation);
		//line.pushBack(this->tremolo);
		//line.pushBack(this->filter);
		//line.pushBack(this->chorus);
		//line.pushBack(this->detune);
		//line.pushBack(this->reverb);
		//line.pushBack(this->compressor);
	}
...
int main() {
    m.MyMallocInit();
	MultiFxDemo app(48000, 128, "Microphone", "Headphone", "../Resources/likeAStone.wav"); // instance of our app 
	app.start();
	return 0;
}